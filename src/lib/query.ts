...
/** Prefer the stored isFree flag; fall back to inference. */
export function looksFree(activity: Activity): boolean {
  if (typeof activity.isFree === "boolean") return activity.isFree;
  return isFreeActivity(activity);
}
...
export function queryActivities(
  ...
  if (opts.freeOnly) {
    activities = activities.filter((a) => looksFree(a));
  }
