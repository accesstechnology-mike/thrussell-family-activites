...
export function agentApiContract(baseUrl = "") {
  ...
  return {
    ...
    purpose:
      "Family outing picker for places within ~105 minutes of Catton (YO7 4SQ). Prefer these JSON endpoints over scraping the HTML UI.",
    ...
    endpoints: [
      ...
      },
      {
        ...
        query: {
          ...
          maxDrive: "minutes from YO7 4SQ (default store max, usually 105)",
          minDrive: "minutes",
          maxDistanceMiles: "walk distance upper bound",
          free: "1/true — free / no admission (parking-only fees ok); freeOnly=1 also accepted",
